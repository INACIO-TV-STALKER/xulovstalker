...
const addon = {
    ...
    async authenticate(config) {
        ...
        try {
            ...
            if (token) {
                ...
                setCache(cacheKey, finalAuth, 60); return finalAuth;
            }
            return null;
        } catch (e) { return null; }
    },
    ...
    async getManifest(configBase64) {
        ...
        await Promise.all(lists.map(async (l, i) => {
            ...
            // AJUSTE: Nome da lista aparece aqui no catálogo (Cabeçalho do Stremio)
        ...
        const m = { id: "org.xulov.stalker", version: "6.1.0", name: "XuloV Hub", resources: ["catalog", "stream", "meta"], types: ["tv", "movie", "series"], idPrefixes: ["xlv:"], catalogs: catalogs };
            ...
            try {
                ...
                if (config) {
                    if (config.type === 'xtream') {
                        ...
                        Object.keys(episodes).forEach(seasonNum => {
                            episodes[seasonNum].forEach(ep => {
                                videos.push({
                                    ...
                                    season: parseInt(seasonNum), number: parseInt(ep.episode_num)
                                });
                            });
                        });
                        ...
                        const auth = await addon.authenticate(config);
                        if (auth) {
                            // DINÂMICO: Trata o ID da série como pasta de episódios
    ...
    async getStreams(type, id, configBase64, host) {
        ...
        // AJUSTE DE LAYOUT: O nome do canal vai no campo "name" (letras grandes)
        // O campo "title" leva apenas a informação do modo (Directo/Proxy) em letras pequenas
        ...
        if (config?.type === 'xtream') {
            const b = config.url.trim().replace(/\/$/, "");
            return { streams: [
                { name: name, url: `${b}/${config.user}/${config.pass}/${sId}`, title: `⚡ Directo`, behaviorHints: { notWebReady: true } },
                { name: name, url: pUrl, title: `🛡️ Proxy`, behaviorHints: { notWebReady: true } }
            ]};
        }
